...
if film
    ...
    writerObj = VideoWriter('Video_efforts_accessibles.mp4', 'MPEG-4');
    writerObj.FrameRate = desired_frame_rate;
    open(writerObj)
    Lims = [-5, 5; -5, 5; -2, 20; -5, 5];
    ...
    for i = 1:number_of_frames
        frames{i} = getframe(Func_visu_movie(Fr_base(:,ceil(i*init_frame_rate/desired_frame_rate)), J_rotor_flat_num, alpha_com(ceil(i*init_frame_rate/desired_frame_rate)), dead_zone_alpha, fprop_sat_vec, Lims));
        
        f = waitbar(i/number_of_frames, f, num2str(i/number_of_frames*100)+" % --- "+num2str(i)+" images sur "+num2str(number_of_frames)+" traitées");
    end
    close(f);
    disp("frames crées")
