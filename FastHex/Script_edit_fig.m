...
if ~cascade
    ...
    if n_h > 0 && size(find(P2C_horizontal), 1) > 0
        ...
        [x_cone, y_cone, z_cone] = cylinder([0, r_cone, 0], N_precision_sphere_cercle);
...
if ~cascade
    legend("Enveloppe forces atteignables", "Double cone simplifié des forces atteignables", 'Interpreter', 'latex', 'Location', 'northeast')
else
...
ylabel('$F_z$', 'Interpreter', 'latex', 'FontSize', 20, 'Rotation', 0);
...
if ~cascade
    legend("Forces atteignables pour $\psi="+num2str(yaw*180/pi)+"$°", "Coupe du cone simplifié", 'Interpreter', 'latex', 'Location', 'northeast')
else
...
set(gcf, "Theme", "light");
nexttile(1)
legend(["Enveloppe forces atteignables", "Double cone simplifié des forces atteignables"], "FontSize", 20, "Interpreter", "latex", "Location", "northeast")
nexttile(2)
legend(["Forces atteignables pour $\psi=0$°", "Coupe du cone simplifié"], "FontSize", 20, "Interpreter", "latex", "Location", "northeast")
hAxes = findobj(gcf,"Type","axes")
hAxes(2).FontSize = 15
hAxes(2).FontWeight = "bold"
hAxes(2).YTick = [-5,-2.5000,0,2.5000,5]
hAxes(2).XTick = [-5,-2.5000,0,2.5000,5]
hAxes(1).FontSize = 15
hAxes(1).FontWeight = "bold"
hAxes(1).XTick = [-5,-2.5000,0,2.5000,5]
Forcesatteignablespourpsi0 = findobj(gcf,"DisplayName","Forces atteignables pour $\psi=0$°")
Forcesatteignablespourpsi0.LineWidth = 2
Coupeduconesimplifi = findobj(gcf,"DisplayName","Coupe du cone simplifié")
Coupeduconesimplifi.LineWidth = 2
Coupeduconesimplifi.LineStyle = ":"
Enveloppeforcesatteignables = findobj(gcf,"DisplayName","Enveloppe forces atteignables")
Enveloppeforcesatteignables.LineStyle = ":"
Enveloppeforcesatteignables.LineWidth = 1